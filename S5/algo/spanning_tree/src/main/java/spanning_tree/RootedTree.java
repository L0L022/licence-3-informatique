...
public class RootedTree {
	...
	private class Node {
		...
		private Node maxHeightSon() {
			Node maxSon = null;
			for (Node son : sons) {
				if (son.height > getHeight(maxSon)) {
					maxSon = son;
				}
			}
			return maxSon;
		...
		private void print() {
			System.out.print("Node " + vertex + ", sons: ");
			for (Node son : sons) {
				System.out.print(son.vertex + " ");
			}
			System.out.println("(height: " + height + ", size: " + size + ", 2nd height: " + this.secondMaxHeight()
					+ ", depth: " + depth + ")");
		}
	...
	private void createTree(int root, ArrayList<Arc> sortedArcs) {
		bfsOrder = new ArrayList<>(order);
		inverseBfsOrder = new ArrayList<>(order);
		nodes = new Node[order];
		nodes[root] = new Node(root);

		bfsOrder.add(nodes[root]);
		for (Arc arc : sortedArcs) {
			createNode(nodes, arc);
			bfsOrder.add(nodes[arc.getDest()]);
		}

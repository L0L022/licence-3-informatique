...
public class Grid {
	...
	public boolean isHorizontal(Edge e) {
		return Math.abs(e.v1 - e.v2) == 1;
	}

	public boolean isVertical(Edge e) {
		return Math.abs(e.v1 - e.v2) == width;
	}

	private void drawLine(int h, BitSet right) {
		for (int i = 0; i < width - 1; i++) {
			System.out.print("o");
			if (right.get(vertexOfCoordinate(i, h))) {
				System.out.print("--");
			} else {
				System.out.print("  ");
			}
	...
	private void drawInterline(int h, BitSet up) {
		for (int i = 0; i < width; i++) {
			if (up.get(vertexOfCoordinate(i, h))) {
				System.out.print("|");
			} else {
				System.out.print(" ");
			}
...
//			System.out.println(e.fromVertex + " -- " + e.toVertex);
			if (isHorizontal(e)) {
				right.set(Math.min(e.v1, e.v2));
			}
			if (isVertical(e)) {
				up.set(Math.min(e.v1, e.v2));
			}
		}

...
public class Labyrinth extends JPanel {
	...
	public Labyrinth(Grid g, RootedTree tree) {
		...
		img = new BufferedImage(side * grid.width, side * grid.height, BufferedImage.TYPE_3BYTE_BGR);
	}
	...
	private Color getVertexColor(int vertex) {
		...
		float hue = (float) (depth % colorGradientCycleLength) / colorGradientCycleLength;
		float saturation = (float) 0.7;
		float brightness = (float) Math.min(100, brightnessSlope * height + minBrightness) / 100;
	...
	private void drawVerticalEdge(Graphics2D g, Edge e) {
		int source = Math.min(e.v1, e.v2);
		int dest = Math.max(e.v1, e.v2);
		...
		gradient = new GradientPaint(xMin, yMin + vertexRadius - 1, getVertexColor(source), xMin,
				yMin + 2 * halfSide - vertexRadius, getVertexColor(dest));
		g.setPaint(gradient);
		g.fill(rect);
	}
	...
	private void drawHorizontalEdge(Graphics2D g, Edge e) {
		...
		GradientPaint gradient = new GradientPaint(xMin + vertexRadius - 1, yMin, getVertexColor(source),
